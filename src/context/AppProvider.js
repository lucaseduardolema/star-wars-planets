...
function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (nameFilter.length > 0) {
      const filteredInfo = tableInfo.filter(({ name }) =>
  ...
  const handleSort = () => {
    const stringsParams = tableInfo.filter((e) =>
      Number.isNaN(Number(e[sortParam]))
    );
    const numberParams = tableInfo.filter(
      (e) => !Number.isNaN(Number(e[sortParam]))
    );
